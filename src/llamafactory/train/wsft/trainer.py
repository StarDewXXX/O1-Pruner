...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def __init__(
        self, ref_model: "PreTrainedModel", finetuning_args: "FinetuningArguments", processor: Optional["ProcessorMixin"], **kwargs
    ) -> None:
    ...
    def reference_concatenated_forward(self, model: "PreTrainedModel", batch: Dict[str, "torch.Tensor"]):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        """
        all_logps, valid_length = self.concatenated_forward(model, inputs)
        all_ref_logps, valid_length = self.reference_concatenated_forward(self.ref_model, inputs)

        all_logps = all_logps / valid_length
        all_ref_logps = all_ref_logps / valid_length

        log_ratio = (all_logps - all_ref_logps)

        advantages = inputs['weight']
